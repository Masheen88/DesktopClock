...
public partial class MainWindow : Window
{
    ...
    private string GetTimeZoneAbbreviation(TimeZoneInfo timeZoneInfo)
    {
        var abbreviation = timeZoneInfo.IsDaylightSavingTime(DateTimeOffset.Now) ? timeZoneInfo.DaylightName : timeZoneInfo.StandardName;
        return abbreviation;
    }
    ...
    public string CurrentTimeOrCountdownString
    {
        get
        {
            var timeZoneAbbreviation = App.GetTimeZoneAbbreviation(_timeZone);
            return IsCountdown
                ? $"{Settings.Default.CountdownTo.Humanize(CurrentTimeInSelectedTimeZone)} ({timeZoneAbbreviation})"
                : $"{CurrentTimeInSelectedTimeZone.ToString(Settings.Default.Format)} ({timeZoneAbbreviation})";
        }
    }
